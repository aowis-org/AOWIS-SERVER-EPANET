...
- INP import of project/global settings, patterns, typed curves, junction/reservoir/tank/pipe topology, pumps, all valve families, controls/rules, and active water-quality analysis configuration with canonical native normalization, UUID-resolved references, explicit completeness diagnostics, and native open-error provenance; broad round-trip proof uses the same in-memory canonicalization for its source-native reference and preserves demand rows at full precision across generated-INP reopen;
...
`conformance` contains native-backed hydraulic and water-quality scenarios. `contract` contains public wrapper contract and regression scenarios. `hydraulic` and `quality` select their respective solver domains. `negative` covers invalid-input and reference hardening. `import` covers INP-to-model reconstruction and import diagnostics. `export` covers INP fidelity. `stress` covers deterministic generated-network differentials. `proof` contains the scenario-manifest, upstream-inventory, field-audit, multi-quality isolation/order checks, independent-run reentrancy proof, and upstream Net1 quality-import equivalence proof. `upstream` always contains the source inventory and can also include the original upstream Boost suite when enabled.
...
|---|---|
| `conformance-import-project-globals-net1` | The vendored upstream Net1 INP opens through native EPANET and reconstructs supported project/global timing, hydraulic, energy, report, and core-topology content into a complete import |
...
| `conformance-import-quality-sources-canonical-units` | CONCEN, MASS, FLOWPACED, and SETPOINT sources are reconstructed with canonical `mg/L` / `mg/min` strengths and UUID-resolved source patterns; native quality and source-mass behavior are compared |
| `conformance-import-quality-mixing-reactions-canonical-units` | Complete-mix, two-compartment, FIFO, and LIFO tanks plus global/entity reactions, independent pipe bulk/wall overrides, tank override ranges, limiting concentration, roughness correlation, zero/positive/negative reaction orders, `ug/L`-to-`mg/L` coefficient scaling, and US wall-reaction length/area canonicalization are reconstructed and native-compared |
...
| `conformance-import-quality-source-trace` | TRACE mode, percent tolerance, quality timestep, non-default relative diffusivity, and the trace source node are reconstructed with UUID resolution, native quality equivalence, and import/export/native-reopen round-trip proof |
...
| `conformance-import-metadata-report-fidelity` | Node/link comments and tags plus the complete representable report configuration are imported from US source units, report thresholds are canonicalized, backend qualifiers are retained, and generated INP persistence is verified |
| `conformance-import-report-setting-threshold-canonical` | A `SETTING BELOW` report filter from a CMH/metres source is retained exactly and survives generated-INP export |
| `conformance-import-report-setting-threshold-diagnostic` | A non-canonical `SETTING BELOW` report filter imports successfully but is explicitly marked incomplete rather than being replayed with incorrect mixed link-setting units |
| `conformance-import-quality-roundtrip-official-net1` | Official Net1 must import completely, match native structural inventory/hydraulics/CHEMICAL quality, export/reopen natively, re-import completely, and match the same source behavior again |
| `conformance-import-quality-roundtrip-official-net2` | Official Net2 extends the same proof to its larger chemical network including source-pattern behavior |
| `conformance-import-quality-roundtrip-official-net3` | Official Net3 extends the same proof to the large source-trace network, pump controls, and source-trace quality timeline |
| `conformance-import-quality-roundtrip-upstream-demand-categories` | The upstream demand-category INP preserves native demand-category inventory plus hydraulic/chemical behavior through export and re-import |
| `conformance-import-quality-roundtrip-upstream-tank-overflow` | The upstream tank-overflow INP preserves overflow topology/inputs and native hydraulic/chemical behavior through export and re-import |
| `conformance-import-roundtrip-upstream-network-builder` | The upstream network-builder INP with QUALITY NONE and missing source map geometry preserves native structural/hydraulic behavior through generated geometry and export/re-import |
| `conformance-import-roundtrip-upstream-network-builder-small` | The upstream small network-builder INP preserves its node/link/pump/tank/curve inventory and native hydraulics through export/re-import |
...
The current importer reconstructs the complete EPANET 2.3 water-quality solver-input surface represented by AOWIS: `NONE`/CHEMICAL/AGE/TRACE analysis configuration, initial chemical/water-age node state, all four CHEMICAL source types, canonical source strengths, UUID-resolved source patterns, all four tank mixing models, and CHEMICAL reaction configuration including independent pipe bulk/wall overrides and tank bulk overrides. Dedicated fixtures and upstream Net1 provide native-vs-imported behavioral proof, and valid quality imports are required to emit no deferred quality diagnostic. Coordinates, ordered link vertices, map labels, and backdrop geometry are now imported into the model's mandatory WGS84 fields using the documented degree/synthetic conversion policy. Node/link comments/tags and the representable `[REPORT]` surface are now imported. Typed report fields retain page/status/flags, selections, enablement/precision, and canonically converted physical thresholds; backend-only FILE and generic qualifiers use the existing report backend-command escape hatch. Non-canonical SETTING thresholds remain an explicit incomplete-import diagnostic because their unit depends on link type. Simple controls and structured rules are imported when their semantics are representable by the current public/model boundary. GPV OPEN/CLOSED controls, reservoir-triggered level controls, and the distinction between explicit OPEN/CLOSED and numeric pump settings are reconstructed explicitly. Toolkit enum combinations that the EPANET INP parser/runtime itself cannot execute remain outside the import contract rather than being treated as supported source syntax. Imported pattern, typed-curve, control, and rule references are resolved to AOWIS UUIDs rather than retained as backend indices.
