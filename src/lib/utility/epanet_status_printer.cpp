...
QString EpanetStatusPrinter::toString(const EpanetStatus &status)
{
    ...
    if (status.stage != EpanetStatusStage::None)
        stream << "Stage:         " << enumLabel(status.stage) << '\n';
    if (status.operation != EpanetStatusOperation::None)
        stream << "Operation:     " << enumKey(status.operation) << '\n';
    if (status.property != EpanetStatusProperty::None)
        stream << "Property:      " << enumLabel(status.property) << '\n';
    if (status.entity.type != EpanetStatusEntityType::None)
    {
