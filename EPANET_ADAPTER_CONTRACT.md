...
Node coordinates, link vertices, map labels, and backdrop bounds use the model's WGS84 coordinate fields. INP import therefore converts EPANET drawing geometry at the adapter boundary instead of adding a second schematic coordinate system to the shared model. Source `[BACKDROP] UNITS DEGREES` geometry is interpreted as WGS84 longitude/latitude when its values are geographically valid. `METERS` geometry is centered at WGS84 0°,0° with metric offsets preserved; `FEET` is first converted to metres; `NONE`, missing units, or invalid degree ranges use the explicit synthetic convention of one source map unit = one metre. GeographicLib `LocalCartesian` performs the synthetic local-metre to WGS84 conversion. All link vertices, labels, backdrop bounds, and backdrop offsets use the same transform. Nodes with no source `[COORDINATES]` entry receive a deterministic schematic WGS84 layout near the applicable origin. Exported `[COORDINATES]`, `[VERTICES]`, and `[BACKDROP]` sections use geographic degrees.
