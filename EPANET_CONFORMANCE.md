...
| Valves | PRV, PSV, PBV, FCV, TCV, GPV, and PCV configuration, curves, statuses, settings, flow, velocity, headloss, regulation state, control membership, and native Error-220 adjacency/topology restrictions with actionable preflight diagnostics. |
...
Importer conformance scenarios open native INP files through EPANET, normalize the live project to CMH/metres through the Toolkit, and verify reconstruction into canonical AOWIS project/global settings, patterns, typed curves, core topology, pumps, valves, simple controls, structured rules, and active water-quality analysis configuration. Broad round-trip proofs use a native reference normalized in memory with the same EPANET unit operations before comparing source reconstruction, then independently compare generated INPs against native reopen/re-import. This keeps canonicalization roundoff distinct from adapter reconstruction errors. Export also restores demand rows from the canonical model at full precision and preserves genuinely unnamed demand categories instead of inventing native-writer labels. Net1 proves native junction/reservoir/tank/pipe readback, endpoint reconstruction, pattern/pump-curve reconstruction, pump-reference integrity, both native simple controls, and full native-vs-imported hydraulic timeline equivalence after control reconstruction. Dedicated US-customary fixtures verify demand, emitter, tank, Darcy-Weisbach, pipe-status, leakage, typed curves, default/reservoir/pump/energy patterns, curve- and constant-power pumps, all seven valve families, all four simple-control trigger types, junction/tank/reservoir trigger nodes, GPV OPEN/CLOSED actions, explicit numeric pump settings, enabled state, IF/AND/OR rule premises, THEN/ELSE actions, priorities, and canonical rule/control quantities before comparing the imported AOWIS hydraulics against direct native runs. Dedicated quality-import fixtures verify `QUALITY NONE`, CHEMICAL `ug/L` to canonical `mg/L`, AGE initial hours, TRACE source-node UUID resolution, mode-specific tolerance, relative diffusivity, quality timestep, all four source types and source patterns, all four tank mixing models, global and entity reactions, independent pipe bulk/wall overrides, tank override ranges, limiting concentration, roughness correlation, negative-order Michaelis-Menten tank kinetics, concentration-basis coefficient conversion, US-to-canonical wall-reaction length/area conversion, and native-vs-imported quality timelines. A final upstream-Net1 quality proof independently imports and executes the real EPANET example network and requires the full native-vs-imported CHEMICAL timeline to agree. Every valid quality-import fixture is also checked for the absence of deferred quality diagnostics, so remaining `complete == false` states cannot be attributed to the quality layer. Metadata/report import additionally verifies node/link comments and tags, selected report entities, page/status/summary/messages/energy flags, field enablement/precision, canonical conversion of physical BELOW/ABOVE thresholds, backend preservation of FILE and generic report qualifiers, and generated-INP persistence. Canonical CMH/metre SETTING thresholds are preserved and export-proven; non-canonical SETTING thresholds are a deliberate explicit incomplete-import case because their numeric unit depends on link type. Import-open failures must retain native error details in structured diagnostics, and incomplete imports must explicitly report omitted source content.

Broad completeness proof then exercises the official Net1, Net2, and Net3 example networks plus upstream demand-category, tank-overflow, and both network-builder fixtures. These scenarios compare native source object inventory against the imported model, compare source-native hydraulics and active quality results against AOWIS, export the imported request, reopen/re-import the generated INP through native EPANET, and repeat the inventory and numerical comparisons. This makes the proof sensitive to both silent import loss and export/reopen drift.
